...
| **Agent B** | AI 引擎 + 智能体编排 | Python FastAPI + LLM SDK + RabbitMQ | ai-engine/ |
...
> **外部 Agent 接入方案（2026-03-19 简化决策）**: 废弃 AAP 自定义协议，核心用 REST API + JWT 认证。Phase 1B 编写「Agent 激活指令」（给 LLM 看的自然语言操作手册 + curl 示例）。MCP Server 作为可选增强延后到 Phase 2。Phase 0 在 API 设计时预留 /api/v1/agent/* 命名空间。
...
| 外部 Agent 接入增加安全面 | 安全风隩 | REST API + JWT 鉴权 + 行为沙箱，Phase 1B 仅邀请制 |
