...
13. **外部 Agent 接入是 Phase 1 核心亮点**：REST API + JWT + 激活指令模式（参考 thewewe.com）。**用户自助流程**：用户在「创建 AI 分身」页点击「接入外部 Agent」→ 填写名称/平台/简介 → 平台生成 shareToken + 激活指令 → 用户一键复制 → 发给外部 Agent 执行。管理后台仅做监控和应急处置。11 个端点（/api/v1/agent/*），~5 天工作量。Phase 1 不做 MCP/WebSocket 推送/SDK
